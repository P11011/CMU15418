...
- 将三维海洋体积离散为二维网格，每个时间步长 $\Delta t$ 进行更新。
...
### 2.4 执行时间分析
- 使用 **4D 块布局**可以减少通信时间，因为它提高了数据局部性。
- **同步开销**主要由于屏障等待。
...
### 7.2 状态转换
- **从共享状态到修改状态的优化**：在 MSI 协议中，当处理器从共享状态写入时需要两次总线事务，而在 MESI 协议中，由于独占状态的引入，这种情况只需要一次总线事务即可完成。
...
## 9. 假共享（False Sharing）
### 9.1 问题描述
- **假共享**发生在两个处理器对不同的地址进行写操作，但这些地址映射到同一个缓存行中。
...
- 如果缓存行未被修改（脏位为 0），目录从内存中返回数据并将请求节点标记为持有该行。
...
- **例子**：两个处理器反复请求相同的缓存行的独占访问权限，但每次在写入之前，另一个处理器获得了总线并请求独占权限，导致双方互相失效对方的数据。
...
- 当缓存行被逐出并且需要写回内存时，使用写回缓冲区可以让处理器在缓存行写回之前就继续执行其他操作。
...
- **总存储顺序（Total Store Ordering, TSO）**：
  - 处理器可以在写入操作之前读取其他地址，但其他处理器在观察到写入之前，不能读取新的值。
...
### 4.3 例子：不同一致性模型的表现
- **四个程序的示例**：
  - 假设两个线程分别对变量 A 和 B 进行读写操作，程序的行为会根据一致性模型的不同而有不同的结果。例如，在顺序一致性模型下，线程之间的操作严格按照时间顺序进行，而在松弛一致性模型下，处理器可以重新排序某些操作以提高效率。
...
- Test-and-Set 指令
  ...
  - **优点**：简单，低存储成本。
  - **缺点**：会产生大量的**一致性流量（Coherence Traffic）**，随着等待处理器数量增加，性能急剧下降。
